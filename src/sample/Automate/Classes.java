...
public class Classes {
    ...
    public ArrayList<Classe> getNextsClasses(State e){
        ...
        ArrayList<Classe> alc = new ArrayList<Classe>();
        for( State ee : ale ){
    ...
    public AF toAFD(){
        ...
        ArrayList<String> Alphabet = res.getAlphabet();
        for( Classe c : this.getAlc() ){
            ArrayList<Classe> alc = this.getNextsClasses(c.getAle().get(0));
            ArrayList<Trans> ts = new ArrayList<Trans>();
            int k=0;
            for( Classe c2 : alc ){
                if( c2 != null ){
                    Trans t = new Trans();
    ...
    public void affiche(){
        System.out.println("start affiche");
        for(Classe classe : getAlc()){
            System.out.println( classe );
            for( State state : classe.getAle() ){
                System.out.println(getNextsClasses(state) );
            }
            System.out.println( "--------" );
        }
        System.out.println("end affiche");
    }

    public AF toAFD2(){

        AF res = new AF(); res.setAlphabet(getAlc().get(0).getAle().get(0).getAf().getAlphabet());
        int i=0;
        for( Classe c : this.getAlc() ){
            State state = new State(); state.setName("e"+i);i++;
            if( c.containFinale() )
